...
def finalize_deployment_file_path(deployment_file: Optional[str]) -> str:
    if deployment_file:
        ...
        if file_extension not in ["json", "yaml", "yml", "j2"]:
            raise Exception('Deployment file should have one of these extensions: [".json", ".yaml", ".yml", ".j2"]')
        ...
        potential_extensions = ["json", "yml", "yaml", "j2"]
